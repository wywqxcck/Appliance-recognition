...
void BTN1_SINGLE_Click_Handler(void* btn)//单击回调函数
{
	if(taskTable[taskIndex].CurrentNum == Interface_Mune){
		OLED_Clear();
		taskIndex = taskstudy+1;//在界面1、2进入目标界面
		taskstudy = 0;
		my_index = 0;
		index_num = 0;
		old_index_num = 0;
		if(taskIndex == Study_Mune){
			
		}
	}
	else if(taskTable[taskIndex].CurrentNum == Study_Mune){//在学习界面发送需要学习的ID
		printf("ID:%d\r\n",taskstudy);
		OLED_Clear();
		send_flage = ING_STUDY;
		printf("ADC = %d\r\n",Cell);
	}
	else if(taskIndex == Set_Mune){
		if(set_flage == YES_SET){
			set_flage = ING_SET;
		}
...
{
	if(taskTable[taskIndex].CurrentNum == Study_Mune){//在学习界面发送需要学习的ID
		OLED_Clear();
		printf("ID:%d\r\n",taskstudy);
		send_flage = CLEAR_STUDY;
	}
}
